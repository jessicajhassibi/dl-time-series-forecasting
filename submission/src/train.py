...
def train_model(
    ...
    scheduler = torch.optim.lr_scheduler.CosineAnnealingLR(
        optimizer, T_max=train_config.num_epochs * len(train_loader)
    )
    ...
    if checkpoint is not None:
        global_step, start_epoch = load_model(
            checkpoint, model, optimizer, scheduler, device
        )
        print(
            f"Loaded model and optimizer weights from checkpoint {checkpoint}.\n"
    ...
    with SummaryWriter(log_dir=log_dir) as writer:
        for epoch in trange(start_epoch, train_config.num_epochs, desc="Training"):
            ...
            for batch_idx, sample in inner_loop:
                ...
                is_validation_step = (
                    (validate_step > 0)
                    and (global_step % validate_step == 0)
                    and (global_step > 0)
                )
                ...
                if is_validation_enabled and (is_validation_step or is_last_batch):
                    ...
                    for metric, value in metrics.items():
                        writer.add_scalar(
                            f"Val/{metric}", value, global_step=global_step
                        )
            ...
            if is_validation_enabled:
                tqdm.write(
                    f"Epoch {epoch}: Best {val_metric_name}: {best_metric_value:.4f}, "
    ...
    if is_validation_enabled:
        print(
            f"Best {val_metric_name}: {best_metric_value:.4f}, "
